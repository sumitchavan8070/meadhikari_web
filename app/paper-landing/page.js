...
//   // Initialize the categoryRefs with an empty array.
...
//                   key={index}
//                   onClick={() => {
//                     // Ensure the ref exists before attempting scroll
...
//                 <div
//                   key={index}
//                   ref={(el) => (categoryRefs.current[index] = el)} // Assign ref to current index
//                   className="mb-12"
//                 >
  ...
  const fetchAllData = async () => {
    try {
      ...
      const enrichedCategories = categories.map((category, index) => {
        ...
        const sortedPapers = papers.sort((a, b) => b.QPYear - a.QPYear);
